...
export async function getReportExport(type: string, from: string, to: string): Promise<ExportResult> {
  ...
  if (parsedType.data === "sales") {
    ...
    const data = await prisma.product.findMany({ where: { storeId: ctx.store.id }, select: { sku: true, barcode: true, name: true, status: true, unit: true, stock: true, minStock: true, costPrice: true, sellingPrice: true, category: { select: { name: true } } }, orderBy: { name: "asc" }, take: MAX_ROWS });
    ...
    const data = await prisma.shift.findMany({ where: { storeId: ctx.store.id, openedAt: { gte: range.start, lt: range.end } }, select: { status: true, openedAt: true, closedAt: true, openingCash: true, expectedCash: true, actualCash: true, cashDifference: true, cashier: { select: { name: true } } }, orderBy: { openedAt: "asc" }, take: MAX_ROWS });
