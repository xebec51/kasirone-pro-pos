  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2"><Button variant="outline" render={<Link href="/dashboard/pos/held" />}>Ditahan</Button><Button variant="outline" render={<Link href={`/dashboard/shifts/${shift.id}`} />}>Lihat Shift</Button></div>
      </div>
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_minmax(360px,0.75fr)]">
        ...
        <aside className="space-y-4 rounded-xl border bg-card p-4 xl:sticky xl:top-20 xl:self-start">
          ...
          <div className="space-y-3 border-t pt-4">
            ...
            <div className="space-y-2"><div className="flex items-center justify-between"><Label>Pembayaran</Label><Button type="button" variant="outline" size="sm" onClick={addPayment} disabled={payments.length >= 10}><Plus className="size-3" aria-hidden="true" />Metode</Button></div>{payments.map((payment) => <div key={payment.id} className="grid grid-cols-[minmax(0,1fr)_auto] gap-2 rounded-lg border p-2 sm:grid-cols-[minmax(0,1fr)_110px_auto]"><Select value={payment.method} onValueChange={(value) => updatePayment(payment.id, { method: value || "CASH" })}><SelectTrigger aria-label="Metode pembayaran" className="col-span-2 w-full sm:col-span-1"><SelectValue /></SelectTrigger><SelectContent>{PAYMENT_METHODS.map((method) => <SelectItem key={method} value={method}>{PAYMENT_METHOD_LABELS[method] ?? method}</SelectItem>)}</SelectContent></Select><Input type="number" min={0} value={payment.amount} onChange={(event) => updatePayment(payment.id, { amount: Number(event.target.value) || 0 })} aria-label="Jumlah pembayaran" /><Button type="button" variant="ghost" size="icon-sm" aria-label="Hapus pembayaran" disabled={payments.length === 1} onClick={() => setPayments((current) => current.filter((item) => item.id !== payment.id))}><Trash2 className="size-4" aria-hidden="true" /></Button>{payment.method !== "CASH" && payment.method !== "STORE_CREDIT" ? <Input className="col-span-2 sm:col-span-3" value={payment.reference} onChange={(event) => updatePayment(payment.id, { reference: event.target.value })} placeholder="Referensi (opsional)" maxLength={150} /> : null}</div>)}</div>
